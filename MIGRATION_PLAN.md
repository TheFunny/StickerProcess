...
持久化配置 `settings.toml`, 提供设置面板:

| 设置项 | 默认 | 说明 |
|---|---|---|
| 输出目录 | `./output` | 启动时记忆上次选择 |
| 最大重试次数 | 3 | 现硬编码 |
| 视频大小上限 | 256 KB | 现硬编码 `VIDEO_MAX_SIZE` |
| 图片大小上限 | 512 KB | 现硬编码 `IMAGE_MAX_SIZE` |
| GIF 系数乘数 | 0.75 | 现硬编码 |
| 时长→系数表 | <1s:1.2 ... ≥8s:0.7 | 改为可编辑 (可选) |
| 目标码率基准 | 256*1024*8 | 高级选项 |
| ffmpeg 路径 | 自动 (PATH/FFMPEG_DIR) | 覆盖项 |
| 主题 | 浅色 | 浅色/深色 |
| 语言 | 中文 | zh/en (低优先) |
| 并行任务数 | 1 | 预留 (见 D4) |
